...
export function VolunteerMapScreen({ navigation }: any) {
  // Using coordinates for New Delhi, India
  const volunteerPosition = { latitude: 28.6139, longitude: 77.2090 };
  const incidentPosition = { latitude: 28.6160, longitude: 77.2120 };
      ...
      <SafeAreaView className="absolute top-0 w-full z-20 px-6 pt-4 flex-row justify-between items-start" pointerEvents="box-none">
        ...
        <View className="bg-white/90 rounded-2xl px-6 py-3 shadow-md flex-col items-center border border-gray-200">
          <View className="flex-row items-baseline">
            <Text className="text-4xl font-bold text-[#4CAF50] leading-none">12</Text>
            <Text className="text-base text-gray-500 font-bold">min</Text>
          </View>
          <Text className="text-xs text-gray-500 tracking-wider font-bold uppercase mt-1">3.2 km</Text>
        </View>
